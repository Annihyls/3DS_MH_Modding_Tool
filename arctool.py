...
def extract(filename, args):
    ...
    with open(arc_index_path, 'wb') as index_file:
        for f in arc.file_list:
            # Convertir f['file'] en str si nécessaire (si c'est un bytearray)
            file_path = f['file'].decode('utf-8') if isinstance(f['file'], bytes) else f['file']
            raw_ext = f['raw_ext'].decode('utf-8') if isinstance(f['raw_ext'], bytes) else f['raw_ext']
            
            # Créer le répertoire pour chaque fichier
            directory = base_path + os.path.dirname(file_path.decode('utf-8'))
            if not os.path.exists(directory):
                os.makedirs(directory)

            # Écrire le fichier
            writeFile(base_path + file_path.decode('utf-8'), f['data'])

            # Écrire l'entrée dans l'index en mode binaire
            index_file.write('{}\t{}\n'.format(file_path.decode('utf-8'), raw_ext).encode('utf-8'))
...
def create(filename, args):
    ...
    if os.path.exists(arc_index_path):
        file_list = readFile(arc_index_path).decode('utf-8')
